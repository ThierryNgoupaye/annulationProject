...
export default function AnnulationClientPage() {
    ...
    const fetchAgencyTrips = async (idAgency) => {
        try {
            const response = await axiosInstance(`/historiques/agence/${idAgency}`);
            if (response.data.data) {
                const voyagesFiltres = response.data.data;
                setCancelledTrips(voyagesFiltres);
                setFilteredTrips(voyagesFiltres);
                setDisplayedTrips(voyagesFiltres.slice(0, 1));
                setVisibleTrips(1);
            } else {
    ...
    const handleDateFilter = () => {
        const filtered = cancelledTrips.filter(trip => {
            ...
            if (trip.date_annulation === null) {
                return false;
            }
            const cancellationDate = new Date(trip.date_annulation);
